...
const translations = {
  id: {
    ...
    infoNote: 'Kualitas 1080p (turun otomatis jika tidak tersedia)',
  ...
  en: {
    ...
    btnVideo: 'Download Video',
    btnAudio: 'Download Audio',
    infoNote: '1080p quality (auto fallback if unavailable)',
...
function applyTranslations(lang) {
  ...
  document.getElementById('siteTagline').textContent = t.tagline;
  document.getElementById('btnVideoText').textContent = t.btnVideo;
  document.getElementById('btnAudioText').textContent = t.btnAudio;
  document.getElementById('infoNote').querySelector('span').textContent = t.infoNote;
  ...
  document.getElementById('stepLabel1').textContent = t.stepVerify;
  document.getElementById('stepLabel2').textContent = t.stepFetch;
  document.getElementById('stepLabel3').textContent = t.stepProcess;
  document.getElementById('stepLabel4').textContent = t.stepReady;
  ...
  document.getElementById('donationDesc').textContent = t.donationDesc;
...
let currentStep = 0;
let currentDownloadType = 'mp4'; // 'mp4' or 'mp3'
...
function startProgressSimulation() {
  ...
  currentProgressInterval = setInterval(() => {
    ...
    if (newStep > currentStep) {
      ...
      if (newStep < 4) {
        document.getElementById(`step${newStep + 1}`).classList.add('active');
      }
      currentStep = newStep;
...
function showLoading(show, type = '') {
  if (show) {
    // Update icon based on download type
    if (type === 'mp4') {
    ...
    if (type === 'mp4') {
      loadingTitle.textContent = currentLang === 'id' ? 'Memproses Video...' : 'Processing Video...';
    } else {
      loadingTitle.textContent = currentLang === 'id' ? 'Memproses Audio...' : 'Processing Audio...';
    }
...
function displayError(message) {
  ...
  const errorText = currentLang === 'id' ? 'Error' : 'Error';
...
function getUrl() {
  ...
  if (!rawUrl) {
    const errorMsg = currentLang === 'id' ? 'Masukkan URL YouTube' : 'Please enter a YouTube URL';
    throw new Error(errorMsg);
  }
  if (!rawUrl.includes('youtube.com') && !rawUrl.includes('youtu.be')) {
    const errorMsg = currentLang === 'id' ? 'URL YouTube tidak valid' : 'Invalid YouTube URL';
    throw new Error(errorMsg);
  }
  return rawUrl;
}
        ...
        <div class="video-info">
          ...
          <div class="details">
            ...
            <div class="meta">
              ...
              <span class="quality-badge"><i class="fas fa-tachometer-alt"></i> ${escapeHtml(quality)}</span>
            </div>
          </div>
        </div>
...
async function handleMp4() {
  try {
    ...
    setTimeout(() => {
      showLoading(false);
      displayError(error.message || (currentLang === 'id' ? 'Gagal memproses video. Silakan coba lagi.' : 'Failed to process video. Please try again.'));
    }, 500);
  }
}
...
async function handleMp3() {
  try {
    ...
    setTimeout(() => {
      showLoading(false);
      displayError(error.message || (currentLang === 'id' ? 'Gagal memproses audio. Silakan coba lagi.' : 'Failed to process audio. Please try again.'));
    }, 500);
  }
}
